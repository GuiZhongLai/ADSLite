...
struct AmsTcpHeader
{
	...
	AmsTcpHeader(const uint32_t numBytes = 0)
		: reserved(0), leLength(bhf::ads::htole(numBytes))
	{
		UNUSED(reserved);
	}
...
struct AoERequestHeader
{
	...
	AoERequestHeader(uint16_t sdoIndex, uint8_t sdoSubIndex,
					 uint32_t dataLength)
		: AoERequestHeader(SDO_UPLOAD,
						   ((uint32_t)sdoIndex) << 16 | sdoSubIndex,
						   dataLength)
	{
	}

	AoERequestHeader(uint32_t indexGroup, uint32_t indexOffset,
					 uint32_t dataLength)
		: leGroup(bhf::ads::htole(indexGroup)), leOffset(bhf::ads::htole(indexOffset)), leLength(bhf::ads::htole(dataLength))
	{
	}
...
{
	AoEReadWriteReqHeader(uint32_t indexGroup, uint32_t indexOffset,
						  uint32_t readLength, uint32_t writeLength)
		: AoERequestHeader(indexGroup, indexOffset, readLength), leWriteLength(bhf::ads::htole(writeLength))
	{
	}
...
struct AdsWriteCtrlRequest
{
	AdsWriteCtrlRequest(uint16_t ads, uint16_t dev, uint32_t dataLength)
		: leAdsState(bhf::ads::htole(ads)), leDevState(bhf::ads::htole(dev)), leLength(bhf::ads::htole(dataLength))
	{
	}
...
{
	AdsAddDeviceNotificationRequest(uint32_t __group, uint32_t __offset,
									uint32_t __length, uint32_t __mode,
									uint32_t __maxDelay,
									uint32_t __cycleTime)
		: leGroup(bhf::ads::htole(__group)), leOffset(bhf::ads::htole(__offset)), leLength(bhf::ads::htole(__length)), leMode(bhf::ads::htole(__mode)), leMaxDelay(bhf::ads::htole(__maxDelay)), leCycleTime(bhf::ads::htole(__cycleTime)), reserved()
	{
		UNUSED(reserved);
	}
...
struct AoEHeader
{
	...
	AoEHeader()
		: leTargetPort(0), leSourcePort(0), leCmdId(0), leStateFlags(0), leLength(0), leErrorCode(0), leInvokeId(0)
	{
	}

	AoEHeader(const AmsNetId &__targetAddr, uint16_t __targetPort,
			  const AmsNetId &__sourceAddr, uint16_t __sourcePort,
			  uint16_t __cmdId, uint32_t __length, uint32_t __invokeId)
		: targetNetId(__targetAddr), leTargetPort(bhf::ads::htole(__targetPort)), sourceNetId(__sourceAddr), leSourcePort(bhf::ads::htole(__sourcePort)), leCmdId(bhf::ads::htole(__cmdId)), leStateFlags(bhf::ads::htole(AMS_REQUEST)), leLength(bhf::ads::htole(__length)), leErrorCode(0), leInvokeId(bhf::ads::htole(__invokeId))
	{
	}
